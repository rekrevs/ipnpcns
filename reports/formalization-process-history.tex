...
\begin{titlepage}
  ...
  \begin{tabularx}{\linewidth}{@{}L{39mm}Y@{}}
    ...
    \textbf{Primary record} &
      Versioned task logs (Wotan), steering decisions (Project Control), Git
      states, compiler and build output, source audit, and a checksum-bounded
      Codex session trace \\
...
through the first report. Wotan---a repository-local task queue with one evidence
log per task---preserves declared proof contracts and completion evidence. Project
Control---a separate ledger of evidence-based steering reviews and owner
decisions---preserves why scope changed. A 50 MB, checksum-bounded Codex session
prefix also preserves the dense work between commits: full additions and diffs for
discarded scratch proofs, compiler diagnostics, theorem-name probes, and migrations
into the permanent modules. This record permits a distinction between ordinary
elaboration failures, genuine mathematical obstructions, architectural changes of
proof, and late verification or typesetting corrections.
...
\section{Actors, tools, and records}
\label{sec:environment}

The names used in this history describe a small project environment, not knowledge
the reader is expected to bring. The environment had one human owner, one continuing
AI-agent task, one Git repository, a pinned Lean toolchain, and two lightweight
repository-local control mechanisms. None of the control mechanisms participates in
Lean's logical trust chain.

\subsection{The actors and the workspace}

The \emph{owner} supplied the paper, set the scientific and publication goals,
authorized continuous autonomous execution, resolved matters of scope and external
publication, and challenged an early claim about missing historical evidence. The
\emph{agent} was Codex, an AI system operating through a continuing task. It could
read and patch files, run shell commands, compile Lean and LaTeX, inspect rendered
pages, and use Git and GitHub under the owner's authorization.

The \emph{repository} was the ordinary filesystem directory containing the paper and,
eventually, all Lean source, reports, evidence records, and Git metadata. Git stored
content-addressed snapshots called commits. A clean commit is therefore evidence of
an accepted file state and its relationship to earlier states; it is not a record of
every attempt made before that state was accepted.

The phrase \emph{session trace} refers to the JSON Lines event log written by Codex
under the user's home directory, conventionally abbreviated \artifact{~/.codex/}.
Each line records an observable event such as a message, command, patch, or tool
result. The raw trace remained private and outside Git. A repository document records
the hash and cutoff of the prefix used here, so the historical sample is reproducible
without publishing the entire operational log.

\subsection{The formal verification toolchain}

\emph{Lean} is an interactive theorem prover. Mathematical definitions and theorem
statements are written in Lean source files; tactics construct proof terms; a small
kernel checks the fully elaborated terms. \emph{mathlib} is the imported,
machine-checked mathematical library providing such foundations as Hilbert spaces,
convexity, matrices, measure theory, and finite combinatorics. \emph{Lake} is Lean's
project and build tool: it resolves the pinned dependency graph and asks Lean to
check the modules.

These roles matter historically. A compiler diagnostic can refute one attempted
term or reveal a missing library interface. A successful clean Lake build establishes
that the accepted modules elaborate and kernel-check together. Neither result alone
shows that the theorem statement faithfully represents the paper. That separate
question is addressed by the source audit.

\subsection{Wotan and Project Control}

\emph{Wotan} was the execution ledger. Its machine-readable file
\artifact{wotan/backlog.json} stored task identifiers, state, size, and dependencies.
For each task, \artifact{wotan/dev-log/T-NNNN.md} recorded the mathematical context,
acceptance criteria, planned verification, implementation notes, obstacles, evidence,
and outcome. Only one task was active at a time. A task could not be called complete
until its status, evidence log, and Git commit agreed.

\emph{Project Control} was the steering ledger, stored in
\artifact{PROJECT-CONTROL.md}. It was invoked at scientific gates, after major
results or falsifications, and when the executable queue became empty. A review
compared the current objective, evidence, assumptions, uncertainty, and remaining
value, then recommended continuing, redirecting, evaluating, preserving, waiting,
or stopping. Decisions requiring authority were recorded separately as owner
decisions and only then translated into Wotan tasks.

The division can be summarized as follows:

\begin{center}
\small
\begin{tabularx}{\linewidth}{@{}L{31mm}L{39mm}Y@{}}
\toprule
\textbf{Mechanism} & \textbf{Question answered} & \textbf{What it cannot establish} \\
\midrule
Wotan & What concrete work is ready, active, blocked, or complete, and what
evidence closes it? & Whether the overall scientific direction is still the best
use of effort \\
Project Control & Given current evidence and uncertainty, should scope continue,
change, close, or await an owner decision? & The truth of a Lean theorem or the
empirical adequacy of the CNS model \\
Lean kernel & Does the elaborated proof term have the exact theorem type under the
recorded assumptions? & Whether the formal contract is the intended reading of the
paper \\
Source audit & Does the formal contract agree with the identified PDF and LaTeX
source? & Whether biological systems satisfy the model assumptions \\
\bottomrule
\end{tabularx}
\end{center}

Wotan and Project Control are project-specific names for transparent files and
rules, not external databases or invisible services. A reader needs no installation
of either system: the relevant records are ordinary versioned JSON and Markdown.
Later sections ask what these mechanisms contributed to the result, but their
definitions and evidentiary limits are fixed here.

\subsection{How to read the evidence citations}

Blue bracketed identifiers such as \ev{E01} refer to the evidence ledger in
\cref{app:evidence-ledger}. The ledger points to task logs, decisions, commits,
audits, build records, and the bounded session trace. Files shown with a typewriter
face are repository paths; a leading \artifact{~/} means the user's home directory.
Commit identifiers denote Git snapshots. The terms \emph{contemporaneous} and
\emph{retrospective} distinguish evidence recorded during proof work from later
analysis of the completed corpus.
