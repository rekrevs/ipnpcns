...
A second purpose is methodological. The report follows one central identity
from paper notation to an exact Lean declaration and explains enough of the
language and trusted checker for a non-Lean reader to inspect the claim. It
also distinguishes source mathematics from explicit formalization choices,
new supporting lemmas, constructive strengthenings, and concrete models added
...
Verification status and source provenance answer different questions. A
theorem may be fully verified while still being a new strengthening supplied
by the formalization; conversely, a formula may be copied directly from the
paper but remain outside the verified scope. The following provenance
vocabulary is therefore used independently of \cref{tab:status}.

\begin{table}[htbp]
\centering
\caption{Provenance of mathematics in the formal development.}
\label{tab:provenance}
\begin{tabularx}{\linewidth}{@{}L{32mm}Y@{}}
\toprule
\textbf{Provenance} & \textbf{Meaning} \\
\midrule
Paper claim & A definition, equation, or theorem stated in the paper and
represented without material change. \\
Explicit interpretation & A domain, closure, sign, boundary, or regularity
condition left implicit in prose and made visible in the formal statement. \\
Formalization lemma & New mathematical infrastructure or decomposition used
to reach a source claim, but not attributed to the paper. \\
Strengthening & A new result that implies, quantifies, or extends the source
claim; its additional content belongs to the formalization. \\
Concrete realization & A particular finite or analytic model that satisfies
an informal interface. It demonstrates consistency and consequences, not
empirical truth or uniqueness of interpretation. \\
\bottomrule
\end{tabularx}
\end{table}

\Cref{tab:provenance-map} gives the high-level division. This division is not
cosmetic: it prevents a machine-checked theorem newly discovered during
formalization from being retrospectively presented as a theorem of the paper.

\begin{center}
\small
\refstepcounter{table}
\label{tab:provenance-map}
\textbf{Table \thetable:} Where the principal mathematical content originates.\par
\vspace{0.5em}
\begin{tabularx}{\linewidth}{@{}L{31mm}Y Y@{}}
\toprule
\textbf{Layer} & \textbf{From the paper} & \textbf{Added or made explicit in Lean} \\
\midrule
Closed-cone algebra & Closed conic hull, polar, projection, rejection,
double rejection, and intersection formulas & A closure-safe representation,
the complete metric-projection and Moreau lemma stack, and named pointwise-to-hull
lifting lemmas \\
Finite subspaces & Matrix and Moore--Penrose renderings of projection,
rejection, and comparison & A general rectangular pseudoinverse construction
from the effective map, including all four Penrose equations and uniqueness \\
Learning & NNLS objective, projected update, and informal convergence under
``standard assumptions'' & Exact KKT equivalence and separate full-rank and
rank-deficient deterministic convergence theorems; stochastic convergence is
not supplied \\
Signals & Finite-window signal and filter equations, wavelet example, and
low-pass approximation & A selected \(L^2\) boundary convention, compact causal
Volterra model, exact orthonormality proofs, and a quantified quadratic membrane
remainder \\
Occupancy and populations & Occupancy approximation, overlap heuristic, active
faces, and circuit equations & An exact finite sampling experiment, uniform
exponential error bound, independent product construction, and sufficient
active-face certificates \\
\bottomrule
\end{tabularx}
\end{center}
...
\subsection{From paper prose to a checked declaration}

Lean is an interactive theorem prover based on dependent type theory
\cite{demoura2015lean}. The useful operational picture is simpler than that
foundation may suggest. A \emph{definition} assigns an exact meaning to a
name. A \emph{theorem declaration} states a proposition as a type. Its proof
is a term of that type. Commands called \emph{tactics} help construct the
term, an \emph{elaborator} resolves notation and omitted arguments, and a small
\emph{kernel} independently checks the fully explicit result. The mathematical
library mathlib contributes previously checked definitions and theorems for
Hilbert spaces, convexity, measure theory, matrices, topology, and finite
combinatorics \cite{mathlib2020}.

The paper's double-rejection identity provides a running example. In the
notation of this report it says
\[
  A\cproj B=A\crej(A\crej B).
\]
Before this can be a Lean proposition, every symbol must have a fixed type and
meaning. The project first declares the two operations:

\begin{lstlisting}[style=leanstyle,language={},caption={Definitions behind the running example, with a helper name inlined.}]
def coneProjection (A B : ClosedCone H) : ClosedCone H :=
  conicHull (metricProjection B '' (A : Set H))

def coneRejection (A B : ClosedCone H) : ClosedCone H :=
  coneProjection A (polar B)
\end{lstlisting}

Thus projection of one cone onto another is not assumed to be a cone: the
pointwise image is deliberately wrapped in the norm-closed conic hull.
Rejection is projection onto the paper's non-positive polar. These choices are
the formal contract for source equations (73)--(75), (98), and (108).

The public theorem then reads:
...
theorem doubleRejection (A B : ClosedCone H) :
  coneProjection A B =
    coneRejection A (coneRejection A B) := by
  -- prove both inclusions between the two closed cones
  apply le_antisymm
  ...
\end{lstlisting}

The parameters \(A\) and \(B\) are closed convex cones in a previously declared
space \(H\). The type-class assumptions on \(H\), printed elsewhere as bracketed
arguments, say that it is a complete real inner-product space. The first colon
separates the theorem's parameters from its proposition; \texttt{:= by} begins
the proof. Equality here is extensional: the two closed cones have exactly the
same members. The line \texttt{apply le\_antisymm} reduces equality of ordered
cone objects to the two inclusions. Later \texttt{rw} steps replace a metric
projection using a proved pointwise lemma. The ellipsis abbreviates this report's
listing, not the checked file: the source contains the complete proof and no
placeholder.

\subsection{What is translated, and what is designed}

The route from the printed equation to the theorem is not mechanical textual
translation. It has five inspectable stages:

\begin{enumerate}[label=\arabic*.]
  \item \textbf{Source reading.} The PDF and LaTeX source identify the intended
    operations, the non-positive polar convention, and the generality of closed
    cones in a Hilbert space.
  \item \textbf{Mathematical restatement.} The equation is separated from its
    biological interpretation and written as equality of closed cones.
  \item \textbf{Representation design.} \texttt{ClosedCone H},
    \texttt{conicHull}, \texttt{polar}, \texttt{coneProjection}, and
    \texttt{coneRejection} decide how the mathematics inhabits Lean and
    mathlib. Closure and completeness become explicit.
  \item \textbf{Proof architecture.} New formalization lemmas establish metric
    projection and Moreau decomposition, prove the pointwise projection
    identity, and lift equality of generator sets through the same closed-hull
    constructor.
  \item \textbf{Kernel certification.} Lean elaborates the declaration and proof
    to a fully explicit term. The kernel checks that term against the imported
    definitions and theorem types.
\end{enumerate}

Stages 1 and 2 aim to preserve paper mathematics. Stage 3 necessarily contains
modeling and library-integration choices. Stage 4 contains genuine mathematics
added by the formalization, even when its purpose is to prove a paper theorem.
Stage 5 verifies derivability of the resulting contract. The independent source
audit closes the loop from stage 5 back to stage 1.

\begin{keyresult}[The guarantee for the running example]
The kernel check establishes that, under the displayed Hilbert-space and
closed-cone assumptions and the project's exact definitions, double rejection
equals conic projection. The source audit establishes that this contract matches
the relevant paper equations. Neither check establishes that a neural circuit
implements the abstract operations or that the model is biologically adequate.
\end{keyresult}

\subsection{How library facts and local proofs compose}

Imports are not appeals to an unchecked handbook. Lean records the types of the
mathlib theorems used by a local proof, and the kernel checks the final term
through that dependency graph. Some low-level implementation details remain in
Lean's trusted computing base---the kernel, elaborated environment, compiler and
runtime used to run the checker---but tactics themselves need not be trusted:
a faulty tactic can fail or construct a rejected term, not make an invalid term
pass the kernel.

This distinction also explains the value of named helper lemmas. A declaration
such as \texttt{metricProjection\_polar\_rejection\_eq} records the central
pointwise step as a reusable mathematical interface. The public theorem can then
be read as a short composition of checked facts rather than as a transcript of
tactic search.
...
\textbf{Provenance.} The matrix and subspace identities in this section are
source claims. Their coordinate-free organization is a formalization choice;
the rank-unrestricted Moore--Penrose construction and uniqueness proof are new
supporting mathematics needed because the pinned library supplied no general
matrix API.
...
\textbf{Provenance.} The paper states the closed-cone and Moreau ingredients
used by its algebra, with Moreau's theorem as classical background. The Lean
project adds a complete local bridge from Hilbert projection to the exact sign
and closure conventions required by the source; these bridge lemmas are not
claimed as new content of the paper.
...
\textbf{Provenance.} The double-rejection and intersection identities are the
paper's headline mathematical claims. The pointwise projection lemma,
closed-hull lifting argument, and decomposition into reusable order lemmas are
formalization contributions that certify those claims without changing their
generality.
...
\textbf{Provenance.} The comparison and deterministic invariance formulas come
from the paper. Lean makes coverage and margin hypotheses explicit and proves
the scalar and Gram inequalities; it does not add the omitted random-embedding
law that would generate those hypotheses.
...
\textbf{Provenance.} The signal equations, wavelet parameters, and low-pass
approximation are source material. Selecting restricted-measure Bochner
\(L^2\), choosing causal truncation, constructing a compact Volterra operator,
and replacing the \(O(\Delta t^2)\) term by a Lipschitz-based bound are explicit
formalization choices and strengthenings.
...
\textbf{Provenance.} The objective, clipped update, and learning interpretation
come from the paper. The exact finite KKT equivalence and the two deterministic
convergence packages are formalization contributions. No stochastic convergence
theorem is attributed to either the source or the Lean development.
...
\textbf{Provenance.} The paper supplies the occupancy and overlap formulas as
model calculations. The explicit uniform finite experiment, edge-case
theorems, exponential approximation error bound, and independent product
construction are constructive strengthenings added by the formalization.
...
\textbf{Provenance.} Active faces, population maps, and primitive circuits are
source concepts. The finite KKT/Moreau certificate is a sufficient
formalization lemma, while approximation quality, support selection, and
biological implementation remain named premises rather than new theorems about
neural tissue.
